...
DIARY_KINDS = {"diary", "moment", "mood", "relationship"}
TASK_KIND = "task"
...
class BucketManager:
    ...
    def __init__(self, config: dict, embedding_engine=None):
        ...
        self.diary_mode_enabled = diary_cfg.get("enabled", config.get("personality", {}).get("mode") == "diary")

    @staticmethod
    def normalize_memory_kind(kind: str | None, bucket_type: str = "dynamic") -> str:
        if bucket_type == "feel":
            return "feel"
    ...
    @staticmethod
    def is_diary_kind(kind: str | None) -> bool:
        return BucketManager.normalize_memory_kind(kind) in DIARY_KINDS

    @staticmethod
    def is_task_kind(kind: str | None) -> bool:
        return BucketManager.normalize_memory_kind(kind) == TASK_KIND

    @staticmethod
    def display_content(bucket: dict) -> str:
        """Compressed/summary body — default for breath/read context."""
        return bucket.get("content", "") or ""

    @staticmethod
    def original_text(bucket: dict) -> str:
        """Full original text archive; only returned on explicit request."""
        meta = bucket.get("metadata", {}) if bucket else {}
        return str(meta.get("original_content") or bucket.get("content", "") or "")

    @staticmethod
    def tags_are_locked(metadata: dict) -> bool:
        return bool(metadata.get("tags_locked", False))
    ...
    async def create(
        ...
        content_is_compressed: bool = False,
        ...
        tags = tags or []
        raw_archive = (original_content or content or "").strip()
        body_content = content if content is not None else raw_archive
        ...
        normalized_kind = self.normalize_memory_kind(memory_kind, bucket_type)
        ...
        metadata = {
            ...
            "last_active": now_iso(),
            "activation_count": 0,
            "original_content": raw_archive,
            "content_compressed": bool(content_is_compressed),
            "tags_locked": bool(tags_locked),
        }
        if normalized_kind == TASK_KIND:
            metadata["task_status"] = task_status or "open"
            if task_due:
        ...
        # body = 压缩版（给 Claude 上下文）；original_content = 原文底档
    ...
    async def update(self, bucket_id: str, **kwargs) -> bool:
        ...
        if "original_content" in kwargs:
            post["original_content"] = str(kwargs["original_content"])
        if "content_is_compressed" in kwargs:
            post["content_compressed"] = bool(kwargs["content_is_compressed"])
        if "tags" in kwargs:
            post["tags"] = kwargs["tags"]
            post["tags_locked"] = True  # 人工改标 → 锁定，重跑不覆盖
        if "tags_locked" in kwargs:
            post["tags_locked"] = bool(kwargs["tags_locked"])
        ...
        if "memory_kind" in kwargs:
            post["memory_kind"] = self.normalize_memory_kind(kwargs["memory_kind"], post.get("type", "dynamic"))
        ...
        if "inferred" in kwargs:
            post["inferred"] = bool(kwargs["inferred"])
        if "session_id" in kwargs:
            post["session_id"] = str(kwargs["session_id"])[:64]
    ...
    async def filter_buckets(
        ...
        """Filter buckets by memory kind, task status, and created date range."""
        buckets = await self.list_all(include_archive=include_archive)
        results = []
        for bucket in buckets:
            meta = bucket.get("metadata", {})
            kind = self.normalize_memory_kind(meta.get("memory_kind"), meta.get("type", "dynamic"))
            if memory_kind:
                if memory_kind == "diary":
                    if not self.is_diary_kind(kind):
                        continue
                elif kind != memory_kind:
                    continue
            if task_status and kind == TASK_KIND:
                status = meta.get("task_status", "open")
                if task_status != "all" and status != task_status:
                    continue
