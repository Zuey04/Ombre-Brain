...
class DecayEngine:
    ...
    def __init__(self, config: dict, bucket_mgr):
        ...
        self.arousal_boost = emotion_cfg.get("arousal_boost", 0.15)
        self.short_term_emotion_mix = emotion_cfg.get("short_term_emotion_mix", 0.15)
        self.long_term_emotion_mix = emotion_cfg.get("long_term_emotion_mix", 0.25)

        retention_cfg = decay_cfg.get("retention", {})
        self.activation_exponent = retention_cfg.get("activation_exponent", 0.35)
        self.importance_multiplier = retention_cfg.get("importance_multiplier", 1.0)

        preservation_cfg = decay_cfg.get("preservation", {})
        self.enable_high_importance = preservation_cfg.get("enable_high_importance", True)
        self.high_importance_threshold = preservation_cfg.get("high_importance_threshold", 7)
        self.enable_fact_tags = preservation_cfg.get("enable_fact_tags", True)
        self.fact_tags = {
            str(t).strip().lower()
            for t in preservation_cfg.get("fact_tags", ["fact", "事实", "重要事实"])
            if str(t).strip()
        }
        self.preservation_score = float(preservation_cfg.get("preservation_score", 500.0))

        diary_cfg = config.get("diary_mode", {})
        self.diary_mode_enabled = diary_cfg.get(
            "enabled", config.get("personality", {}).get("mode") == "diary"
        )
        self.disable_auto_resolve = diary_cfg.get("disable_auto_resolve", True)
        self.disable_urgency_boost = diary_cfg.get("disable_urgency_boost", True)
    ...
    def _has_fact_tag(self, metadata: dict) -> bool:
        """Check if bucket tags indicate a preserved factual memory."""
        if not self.enable_fact_tags or not self.fact_tags:
            return False
        tags = metadata.get("tags", [])
        if not isinstance(tags, list):
            return False
        normalized = {str(t).strip().lower() for t in tags if str(t).strip()}
        return bool(normalized & self.fact_tags)

    def _is_auto_preserved(self, metadata: dict) -> bool:
        """High-importance or fact-tagged memories resist decay (configurable)."""
        if self.enable_high_importance:
            try:
                imp = int(metadata.get("importance", 5))
            except (ValueError, TypeError):
                imp = 5
            if imp >= self.high_importance_threshold:
                return True
        return self._has_fact_tag(metadata)
    ...
    def calculate_score(self, metadata: dict) -> float:
        ...
        # --- Auto-preserved: high importance or fact tags ---
        if self._is_auto_preserved(metadata):
            return self.preservation_score

        memory_kind = str(metadata.get("memory_kind", "diary")).lower()
        ...
        # --- Short-term vs long-term: time dominates, emotion is secondary ---
        emotion_mix_short = max(0.0, min(1.0, self.short_term_emotion_mix))
        emotion_mix_long = max(0.0, min(1.0, self.long_term_emotion_mix))
        if days_since <= 3.0:
            combined_weight = (
                time_weight * (1.0 - emotion_mix_short)
                + emotion_weight * emotion_mix_short
            )
        else:
            combined_weight = (
                time_weight * (1.0 - emotion_mix_long)
                + emotion_weight * emotion_mix_long
            )
        ...
        if memory_kind == "task":
            task_status = metadata.get("task_status", "open")
            if task_status == "done":
                ...
                resolved_factor = 1.0
            # Due-soon open tasks get a gentle boost in diary mode
            if self.diary_mode_enabled and task_status == "open":
                due = str(metadata.get("task_due", "")).strip()
                if due:
                    resolved_factor *= 1.3
        elif self.diary_mode_enabled:
            # Diary entries don't use task-style resolved semantics
            resolved_factor = 1.0
        elif resolved and digested:
            resolved_factor = 0.02
        elif resolved:
            resolved_factor = 0.05
        else:
        ...
        if self.diary_mode_enabled and self.disable_urgency_boost:
            urgency_boost = 1.0
        elif memory_kind == "task" and metadata.get("task_status", "open") == "open":
            urgency_boost = 1.3 if arousal > 0.5 else 1.0
        else:
    ...
    async def run_decay_cycle(self) -> dict:
        ...
        for bucket in buckets:
            ...
            # --- Auto-resolve: disabled in diary mode ---
            should_auto_resolve = (
                not (self.diary_mode_enabled and self.disable_auto_resolve)
                and not meta.get("resolved", False)
            )
