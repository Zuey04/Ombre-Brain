...
def _effective_merge_threshold() -> int:
    if DIARY_MODE:
...
def _bucket_api_dict(b: dict, include_content: bool = False) -> dict:
    ...
    data = {
        ...
        "memory_kind": meta.get("memory_kind", "diary"),
        ...
        "content_compressed": bool(meta.get("content_compressed", False)),
        "tags_locked": bool(meta.get("tags_locked", False)),
    ...
    if include_content:
        data["content"] = strip_wikilinks(bucket_mgr.display_content(b))
    else:
        data["content_preview"] = strip_wikilinks(bucket_mgr.display_content(b))[:200]
    return data


def _days_since_created(meta: dict) -> float:
    from datetime import datetime
    created_str = meta.get("created", meta.get("last_active", ""))
    try:
        created = datetime.fromisoformat(str(created_str))
        return max(0.0, (datetime.now() - created).total_seconds() / 86400)
...
# =============================================================
async def _storage_body(raw: str) -> tuple[str, str, bool]:
    """Prepare bucket body (compressed) + original archive from raw input."""
    raw = (raw or "").strip()
    if not raw:
        return "", "", False
    body, compressed = await dehydrator.compress_for_storage(raw)
    return body, raw, compressed
...
async def _merge_or_create(
    ...
    返回 (桶ID或名称, 是否合并)。
    """
    memory_kind = bucket_mgr.normalize_memory_kind(memory_kind)
    try:
    ...
    merge_threshold = _effective_merge_threshold()
    for candidate in existing:
        if candidate.get("score", 0) <= merge_threshold:
            break
        bucket = candidate
        existing_kind = bucket_mgr.normalize_memory_kind(
            bucket["metadata"].get("memory_kind"), bucket["metadata"].get("type", "dynamic")
        )
        if existing_kind != memory_kind:
            continue
        if bucket["metadata"].get("pinned") or bucket["metadata"].get("protected"):
            continue
        try:
            raw_new = content.strip()
            old_original = bucket_mgr.original_text(bucket)
            merged_original = f"{old_original}\n---\n{raw_new}" if old_original else raw_new
            new_body_part, _ = await dehydrator.compress_for_storage(raw_new)
            merge_input = new_body_part if new_body_part else raw_new
            merged_body = await dehydrator.merge(bucket["content"], merge_input)
            ...
            update_kwargs = {
                "content": merged_body,
                "original_content": merged_original,
                "content_is_compressed": True,
                "importance": max(bucket["metadata"].get("importance", 5), importance),
                "domain": list(set(bucket["metadata"].get("domain", []) + domain)),
                "valence": merged_valence,
                "arousal": merged_arousal,
            }
            if not bucket_mgr.tags_are_locked(bucket["metadata"]):
                update_kwargs["tags"] = list(set(bucket["metadata"].get("tags", []) + tags))
            if memory_kind == "task" and task_due:
...
async def _build_diary_surfacing(max_tokens: int, max_results: int) -> str:
    """Diary-first surfacing: recent life moments + a few open tasks."""
    try:
        all_buckets = await bucket_mgr.list_all(include_archive=False)
    except Exception as e:
        logger.error(f"Failed to list buckets for diary surfacing: {e}")
        return "记忆系统暂时无法访问。"
    ...
    for b in pinned_buckets:
        try:
            ...
            logger.warning(f"Pinned dehydrate failed: {e}")

    diary_candidates = [
        b for b in all_buckets
        if bucket_mgr.is_diary_kind(b["metadata"].get("memory_kind", "diary"))
        and b["metadata"].get("type") not in ("permanent", "feel")
        and not b["metadata"].get("pinned", False)
        and not b["metadata"].get("protected", False)
        and _days_since_created(b["metadata"]) <= SURFACE_RECENT_DAYS
    ]
    diary_scored = sorted(
        diary_candidates,
    ...
    open_tasks = [
        b for b in all_buckets
        if bucket_mgr.is_task_kind(b["metadata"].get("memory_kind"))
        and b["metadata"].get("task_status", "open") == "open"
        and b["metadata"].get("type") not in ("permanent", "feel", "archived")
    ]
    open_tasks.sort(
        key=lambda b: (
            0 if b["metadata"].get("task_due") else 1,
            b["metadata"].get("task_due", "9999"),
    ...
    for b in diary_scored:
        ...
        try:
            ...
            kind = b["metadata"].get("memory_kind", "diary")
            diary_results.append(f"[{kind}] [权重:{score:.2f}] [bucket_id:{b['id']}] {summary}")
            token_budget -= t
        except Exception as e:
            logger.warning(f"Diary surfacing dehydrate failed: {e}")
    ...
    for b in open_tasks:
        if token_budget <= 0:
            break
        meta = b["metadata"]
        due = meta.get("task_due", "")
        quote = meta.get("source_quote", "")
        line = f"☑️ [task] [bucket_id:{b['id']}] {meta.get('name', b['id'])}"
        if due:
            line += f" (due:{due})"
    ...
    if not pinned_results and not diary_results and not task_results:
        return "最近平静，没有特别想提的事。"
...
async def _build_task_surfacing(max_tokens: int, max_results: int) -> str:
    try:
        tasks = await bucket_mgr.filter_buckets(memory_kind="task", task_status="open")
    except Exception as e:
        return f"读取待办失败: {e}"
    if not tasks:
        return "没有进行中的待办。"
    ...
    results = []
    token_used = 0
    for b in tasks[:max_results]:
        meta = b["metadata"]
        line = (
            f"☑️ [bucket_id:{b['id']}] {meta.get('name', b['id'])} "
            f"status:{meta.get('task_status', 'open')}"
        )
        if meta.get("task_due"):
            line += f" due:{meta['task_due']}"
        line += f"\n{strip_wikilinks(b['content'][:300])}"
        if meta.get("source_quote"):
            line += f"\n原话: {meta['source_quote'][:120]}"
        t = count_tokens_approx(line)
...
async def breath(
    ...
    """检索/浮现记忆。mode: recent/diary/tasks/feel(日记模式默认recent)。不传query=自动浮现。domain=feel读取feel。task用mode=tasks。"""
    ...
    if not query or not query.strip():
        if mode == "tasks":
            return await _build_task_surfacing(max_tokens, max_results)
        if mode in ("recent", "diary") or (DIARY_MODE and mode != "unresolved"):
            return await _build_diary_surfacing(max_tokens, max_results)
    ...
    # --- Feel retrieval ---
    if mode == "feel" or domain.strip().lower() == "feel":
        try:
            all_buckets = await bucket_mgr.list_all(include_archive=False)
...
async def hold(
    ...
    final_kind = bucket_mgr.normalize_memory_kind(memory_kind or analysis.get("memory_kind", "diary"))
    final_importance = importance if 1 <= importance <= 10 else int(analysis.get("importance", DEFAULT_DIARY_IMPORTANCE))
    if final_kind == "task" and final_importance < 5:
        final_importance = 5
    elif bucket_mgr.is_diary_kind(final_kind) and final_importance > 6:
        final_importance = min(final_importance, 6)
...
async def grow(content: str) -> str:
    ...
    if len(content.strip()) < 30:
        ...
        result_name, is_merged = await _merge_or_create(
            ...
            importance=int(analysis.get("importance", DEFAULT_DIARY_IMPORTANCE)),
    ...
    for item in items:
        try:
            result_name, is_merged = await _merge_or_create(
                ...
                tags=item.get("tags", []),
                importance=item.get("importance", DEFAULT_DIARY_IMPORTANCE),
            ...
            if is_merged:
                ...
                results.append(f"{icon}{item.get('name', result_name)}")
                created += 1
        except Exception as e:
            logger.warning(
...
async def trace(
    ...
    """修改记忆元数据。task_status=open/done/cancelled 用于待办。resolved仅task模式常用。delete=True删除。"""
...
async def pulse(include_archive: bool = False) -> str:
    ...
    for b in buckets:
        ...
        if meta.get("pinned") or meta.get("protected"):
            ...
            icon = "🫧"
        elif bucket_mgr.is_task_kind(meta.get("memory_kind")):
            icon = "☑️" if meta.get("task_status", "open") == "open" else "✅"
...
async def dream() -> str:
    ...
    header = (
        ...
        "- 有什么感受值得写下来？\n"
        "- 有什么待办用户可能忘了？（轻量提醒即可）\n"
        "想完之后：有沉淀的用 hold(feel=True) 写感受；\n"
        "用户明确说「搞定了」才 trace(task_status=\"done\") 或 trace(resolved=1)。\n"
        "日记式记忆不必强行结案。没有沉淀就不写。\n"
    )
...
async def api_bucket_detail(request):
    ...
    include_original = request.query_params.get("include_original", "").lower() in ("1", "true", "yes")
    payload = {
        "id": bucket["id"],
        "metadata": meta,
        "content": strip_wikilinks(bucket_mgr.display_content(bucket)),
        "content_compressed": bool(meta.get("content_compressed", False)),
        "score": decay_engine.calculate_score(meta),
    }
    if include_original:
        payload["original_content"] = strip_wikilinks(bucket_mgr.original_text(bucket))
    return JSONResponse(payload)


@mcp.custom_route("/api/timeline", methods=["GET"])
async def api_timeline(request):
    """Chronological feed for frontend: diary + tasks mixed."""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err:
        return err
    ...
    try:
        ...
        items = [_bucket_api_dict(b) for b in buckets]
...
async def api_diary_create(request):
    """Alias of POST /api/diary-ledger (App 保存日终日记)."""
    return await api_create_diary_ledger(request)
...
async def api_tasks(request):
    ...
    try:
        buckets = await bucket_mgr.filter_buckets(
            memory_kind="task",
            task_status=status if status else "all",
        )
        items = [_bucket_api_dict(b, include_content=True) for b in buckets]
        items.sort(
            key=lambda x: (
                0 if x.get("task_due") else 1,
                x.get("task_due", "9999"),
                x.get("created", ""),
            )
        )
...
@mcp.custom_route("/api/tasks/{bucket_id}", methods=["PATCH"])
async def api_task_update(request):
    """Update task status from frontend."""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err:
        return err
    ...
    return JSONResponse(_bucket_api_dict(updated, include_content=True))
